---
id: alpine-moodle-reverse-proxy
title: 🔒 Reverse Proxy
description: How to configure a reverse proxy for Moodle
slug: /alpine-moodle/reverse-proxy
sidebar_position: 4
---
...
Setting `REVERSEPROXY=true` on a single-URL deployment triggers _"Reverse proxy enabled so the server cannot be accessed directly"_ errors (issue [#137](https://github.com/erseco/alpine-moodle/issues/137)). Leave it at `false` unless you actually serve the same site from several hostnames.
:::
...
To log the real visitor IP instead of the Cloudflare edge IP, enable the `CF-Connecting-IP` header in your front proxy (or use Cloudflare's _True-Client-IP_) and ensure the next proxy hop copies it into `X-Forwarded-For`.

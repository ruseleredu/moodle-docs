---
id: alpine-moodle-upgrading
title: 🔄 Upgrading
description: How to upgrade Moodle in the Alpine container
slug: /alpine-moodle/upgrading
sidebar_position: 10
---
...
<Tabs>
  <TabItem value="Without" label="Without `moodlehtml` volume" default>
  ...
  <TabItem value="moodlehtml" label="With `moodlehtml` volume">
    ...
    1. Back up the database, `moodledata`, and `moodlehtml` (see [Persistence & Volumes](persistence.mdx)).
...
If you see _"`/var/www/html/vendor/composer` does not exist"_ ([#117](https://github.com/erseco/alpine-moodle/issues/117)), the container has not finished bootstrapping yet. Watch `docker compose logs -f moodle` — the error is transient unless it recurs after 30+ seconds.
...
Find docker running instances:

```bash
docker compose ls
```

Change directory to `docker-compose.yml` location and pull:
...
```bash
docker compose exec moodle cat public/version.php
```

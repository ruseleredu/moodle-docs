...
- Put the container [behind a reverse proxy](reverse-proxy.mdx) for HTTPS.
- Review the [environment variables](environment-variables.mdx) to tune PHP limits, SMTP, Redis, etc.
- Read [Persistence & Volumes](persistence.mdx) before your first backup.
- When upgrading Moodle, follow [Upgrading](upgrading.mdx).

---
id: alpine-moodle-faq
title: ❓FAQ
description: Frequently asked questions about the Alpine Moodle container
slug: /alpine-moodle/faq
sidebar_position: 11
---
...
`amd64`, `arm64`, `arm/v7`, `arm/v6`, `386`, `ppc64le`, `s390x`. Pull `ruseler/alpine-moodle` on any of them and Docker will fetch the right variant.

## Does the image include Redis / PostgreSQL / MariaDB?

No. The image only contains Moodle, PHP 8.3, Nginx, Moosh and supporting tools. External services (database, Redis) must run in their own containers. See [Docker Compose](docker-compose.mdx) for ready-made stacks.
...
In `/var/www/moodledata/`. Mount a volume there to persist them. See [Persistence & Volumes](persistence.mdx).

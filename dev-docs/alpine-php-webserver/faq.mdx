...
```bash
docker run -p 80:8080 ruseler/alpine-php-webserver
```
...
```dockerfile
FROM ruseler/alpine-php-webserver:latest
USER root
...
Yes: `amd64`, `arm64`, `arm/v7`, `arm/v6`, `386`, `ppc64le`, `s390x`. Just pull `ruseler/alpine-php-webserver` on any of them.

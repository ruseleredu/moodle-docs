...
You don't install it into this image — you build your own thin image on top with `apk add --no-cache composer`. See [Composer & Building](composer.mdxx) for the recipe.
...
Environment variables. The entrypoint expands `/etc/php84/conf.d/custom.ini.tpl` through `envsubst`. See [PHP Configuration](php.mdxx) and the full [Environment Variables](environment-variables.mdxx) reference.
...
Used for installing debug tools, inspecting file ownership, or running one-off apk commands. See [Extending the Image](extending.mdxx).

## Can I run additional services (queue worker, supervisor) in the same container?

Yes — add a `runit` entry. Each daemon lives in `/etc/service/<name>/run`. See [Extending the Image](extending.mdxx) for the full pattern, or read the bundled `nginx` and `php` service scripts for working examples.
...
And **restart the container on every deploy** — with `validate_timestamps=0`, PHP will not re-read changed files on its own. See [PHP Configuration](php.mdxx).

## How do I get the real client IP?

Set `REAL_IP_FROM` to the list of trusted proxies. Real IP restoration is disabled by default for safety. See [Reverse Proxy & Trusted IPs](reverse-proxy.mdxx).

...
```bash
docker run -p 8080:8080 -v "$PWD/app:/var/www/html" ruseler/alpine-php-webserver
```
...
**Fix**: `docker pull ruseler/alpine-php-webserver` to update. If you really need to stay on an older tag, mount a replacement `server-conf.d/` snippet that overrides the default location block — or set `DISABLE_DEFAULT_LOCATION=true` and provide your own.

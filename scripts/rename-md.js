...
// const dirs = ['./docs', './blog', './hil', './win', './ti'];
const targetDirs = [
    path.join(projectRoot, "docs"),
    path.join(projectRoot, "blog"),
    path.join(projectRoot, "dev-docs"),
    path.join(projectRoot, "edu-docs"),
    path.join(projectRoot, "qa-docs"),
    path.join(projectRoot, "adm-docs"),
];

const renameFiles = (dir) => {
    if (!fs.existsSync(dir)) {
        console.log(
            `\x1b[33mℹ Directory not found (skipping):\x1b[0m ${path.relative(projectRoot, dir)}`,
        );
        return;
    }
    ...
    files.forEach((file) => {
        ...
        if (stat.isDirectory()) {
            ...
            if (!fs.existsSync(newPath)) {
                ...
                console.log(
                    `✅ Renamed: ${path.relative(projectRoot, oldPath)} -> ${path.basename(newPath)}`,
                );
            } else {
                console.warn(
                    `⚠️ Collision: ${path.basename(newPath)} already exists. Skipped ${file}`,
                );
            }
        }
    });
